...
sayHelloButton.addEventListener('mouseout', helloBtn);
...
/*
    ...
    the callback function, console.log the response's data (in the intermediate instructions we'll come back to 
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/myparam')
    .then(res => {
        let content = document.getElementById('repeat-text')
...
function queryTest() {
    axios.get('http://localhost:3000/query-test/?query=testing')
    .then(res => {
        console.log(res.data)
    })
    .catch(err => {
...
/*
    ...
    1: Send no queries on the URL -- what happened? it states that you returned an empty query

    2: Send more than 1 query on the URL -- what happened? It adds the second query as a string onto the value of the first query
*/
...
//PROBLEM 11
/*
    You are going to add the ability to POST to the server. You'll need to create a small form and write a function 
    that makes a post request. Then you'll attach that function to the submit event on the form. We'll be creating 
    a list of foods. 
    ...
    Use a .then to handle the promise returned from the axios call. Pass a callback function to the .then. Inside 
    that callback, console log the res.data. 

    Based on what we did earlier to display this type of data, write code that will display the response in your 
    HTML document. 
*/
...
theButton.addEventListener('click', createFood);